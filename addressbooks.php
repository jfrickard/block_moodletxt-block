...
if (count($addressbookOutputSet) > 0) {
    ...
    $clearer = html_writer::tag('div', '', array('class' => 'cleared'));
    
    foreach($addressbookOutputSet as $addressbookEntry) {
        $className = ($addressbookEntry->getType() == MoodletxtAddressbook::$ADDRESSBOOK_TYPE_GLOBAL) ? 'globalAddressbook' : 'privateAddressbook';
