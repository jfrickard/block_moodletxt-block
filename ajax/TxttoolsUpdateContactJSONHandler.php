...
class TxttoolsUpdateContactJSONHandler {
    ...
    public function processJSON($json) {
        ...
        switch($decoded->mode) {
            ...
            case 'updateContact':
                $response = $this->updateContact((int) $decoded->addressbookId, $decoded->contact);
                break;
     ...
     * @param object $contactJSON Contact data object constructed from JSON
     * @return string JSON-encoded response to request
     * @throws MoodletxtAJAXException
     * @version 2012090501
     * @since 2012090501
     */
    private function updateContact($addressbookId, $contactJSON) {
        ...
        try {
            $contact = $this->addressbookDAO->getAddressbookContactById($addressbookId, $contactJSON->id);
            
            $contact->setFirstName((string) $contactJSON->firstName);
            $contact->setLastName((string) $contactJSON->lastName);
            $contact->setCompanyName((string) $contactJSON->company);
            $contact->setRecipientNumber(new MoodletxtPhoneNumber((string) $contactJSON->phoneNo));
