...
class TxttoolsAccountJSONHandler {
    ...
    public function processJSON($json) {
        ...
        switch($decoded->mode) {
            ...
            case 'updateAccountFromUser':
                $response = $this->updateAccountFromUser($decoded);
                break;
     ...
     * @since 2012100801
     */
    private function updateAccountFromUser($decodedJson) {
        ...
        if (!is_object($txttoolsAccount))
            throw new MoodletxtAJAXException(
                get_string('errorinvalidaccountid', 'block_moodletxt'), 
                MoodletxtAJAXException::$ERROR_CODE_BAD_ACCOUNT_ID,
                null, false, $decodedJson->accountId);
        
        $txttoolsAccount->setDescription($decodedJson->description);
        ...
        if ($decodedJson->newPassword != '') {
            
            $encrypter = new Encryption();
            $key = get_config('moodletxt', 'EK');

            $txttoolsAccount->setEncryptedPassword($encrypter->encrypt($key, $decodedJson->newPassword, 20));
